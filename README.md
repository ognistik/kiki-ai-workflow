...
* **Orange-colored snippets** simulate the "CTRL + SHIFT + A" keyboard shortcut to select text up to the previous line break. **IMPORTANT: while most text editing apps receive and interpret this shortcut, it does not work in every app. It’s a good idea to test first.** The snippets in Kiki will send the selected text to one of the options available as Universal Actions. **With snippets, along with the same options found in Universal Actions, you can also directly process text presets and automatically paste the results.** It’s a fantastic way to use Kiki inline, without loosing focus on what you are typing. More information about this and how to create your own can be found in the Advanced section.
